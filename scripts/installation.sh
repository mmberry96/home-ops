...
function apply_talos_config() {
  ...
  for file in ${talos_controlplane_file} ${talos_worker_file}; do
    ...
    for node in ${nodes}; do
      ...
      if ! output=$(echo "${resources}" | talosctl --nodes "${node}" apply-config --config-patch "@${node_patch_file}" --insecure --file /dev/stdin 2>&1); then
...
function wipe_rook_disks() {
  ...
  for node in ${nodes}; do
    if ! disks=$(talosctl --nodes "${node}" get disk --output json 2>/dev/null |
      jq --exit-status --raw-output --slurp '. | map(select(.spec.model == env.ROOK_DISK) | .metadata.id) | join(" ")') || [[ -z "${disks}" ]]; then
